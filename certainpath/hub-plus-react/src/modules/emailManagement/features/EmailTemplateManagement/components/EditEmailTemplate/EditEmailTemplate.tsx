...
import { useEditEmailTemplate } from "@/modules/emailManagement/features/EmailTemplateManagement/hooks/useEditEmailTemplate";
...
function EditEmailTemplate() {
  ...
  useEffect(() => {
    if (emailTemplateId) {
      fetchEmailTemplate(Number(emailTemplateId));
    }
  }, [emailTemplateId, fetchEmailTemplate]);

  const manualBreadcrumbs = useMemo(() => {
    if (!emailTemplateId) return undefined;
    const editTemplateName =
      templateName || `Email Template ${emailTemplateId}`;
    return [
      { path: "/email-management/email-templates", label: "Email Templates" },
      {
        path: `/email-management/email-templates/${emailTemplateId}/edit`,
        label: `Editing ${editTemplateName}`,
        clickable: false,
      },
    ];
        ...
        <Form {...form}>
          ...
          >
            <fieldset>
              <Card>
                <CardHeader>
                  <CardTitle className="text-xl">Edit Email Template</CardTitle>
                ...
                <CardContent className="space-y-6">
                              ...
                              <EntityMultiSelect
                                ...
                                renderEntityRow={(ent, isSelected, toggle) => {
                                  return (
                                    <div
                                      className={`
                                        cursor-pointer flex items-center justify-between py-4 px-2 
                  ...
                  <div className="sticky bottom-0 flex justify-end gap-4 px-4 py-4 border-t bg-white mt-8">
                    <Button
                      onClick={handleCancelEditEmailTemplate}
                      type="button"
                      variant="outline"
                    >
                      Cancel

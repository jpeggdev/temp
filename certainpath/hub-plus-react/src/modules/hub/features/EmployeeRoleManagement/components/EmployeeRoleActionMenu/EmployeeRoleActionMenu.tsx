...
interface EmployeeRoleActionMenuProps {
  roleId: number;
  onEditRole: (id: number) => void;
  onDeleteRole: (id: number) => void;
}

const EmployeeRoleActionMenu: React.FC<EmployeeRoleActionMenuProps> = ({
  roleId,
  onEditRole,
  onDeleteRole,
}) => {
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-[150px] p-2 bg-white">
        <DropdownMenuItem
          ...
          onClick={() => onEditRole(roleId)}
        >
          Edit Role
        </DropdownMenuItem>
        <DropdownMenuItem
          className="flex items-center px-3 py-2 text-sm cursor-pointer hover:bg-gray-100 transition-colors"
          onClick={() => onDeleteRole(roleId)}
        >
          Delete Role
        </DropdownMenuItem>
      </DropdownMenuContent>
...
export default EmployeeRoleActionMenu;

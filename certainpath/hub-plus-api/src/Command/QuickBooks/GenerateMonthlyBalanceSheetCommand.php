...
use App\Service\CreateMonthlyBalanceSheetReportService;
use App\SQL\GetMonthlyBalanceSheetsSQL;
...
#[AsCommand(
    name: 'app:quickbooks:generate-monthly-balance-sheet',
    description: 'Generate monthly balance sheet reports for QuickBooks Reporting.',
)]
class GenerateMonthlyBalanceSheetCommand extends Command
{
    public function __construct(
        private readonly GetMonthlyBalanceSheetsSQL $getMonthlyBalanceSheetsSQL,
        private readonly CreateMonthlyBalanceSheetReportService $createMonthlyBalanceSheetReportService,
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Generating Monthly Balance Sheet Reports');
        ...
        if ($tenant) {
            ...
            $io->text('Processing all unprocessed reports for all companies with an Intacct ID.');
        ...
        $io->success('All applicable reports processed successfully.');
    ...
    {
        $processedReports = $this->quickbooksReportRepository->findAll();
        $processedReportKeys = [];
        foreach ($processedReports as $processedReport) {
            $key = $processedReport->getIntacctId().'|'.$processedReport->getReportId();
            $processedReportKeys[$key] = true;
        }

        $allReports = $this->getMonthlyBalanceSheetsSQL->getAllUniqueReports();
        $unprocessedReports = [];
        foreach ($allReports as $report) {
            $key = $report['tenant'].'|'.$report['report_id'];
            if (isset($processedReportKeys[$key])) {
                continue;
            }
    ...
    private function processReport(string $tenant, string $reportId, SymfonyStyle $io): void
    {
        try {
            ...
            $rawData = $this->getMonthlyBalanceSheetsSQL->execute($tenant, $reportId);
            if (empty($rawData)) {
            ...
            $excelContent = $this->createMonthlyBalanceSheetReportService->generateReport($rawData);
            $s3Key = 'reports/excel/monthly_balance_sheet/'.$tenant.'_'.$reportId.'.xlsx';
            ...
            $reportEntity->setReportType(ReportType::MONTHLY_BALANCE_SHEET);
            ...
            $this->quickbooksReportingLogger->error(
                "QuickBooksReportingCommand: Error processing report for Tenant: $tenant, Report ID: $reportId",
                [
                    'exception_message' => $e->getMessage(),
                    'tenant' => $tenant,
                    'report_id' => $reportId,
